...
# best_chromo ： 每一代最优的染色体
# best_score ： 最优染色体对应的分数
def generations_v2(X_train,X_train_val,y_train,y_train_val,X_test,y_test,model,pop_size,num_features,drop_rate = 0.3,mutation_rate=0.3,num_generation=10,err_tolerance=0,name=None):
    ...
    for i in range(num_generation):
        ...
        plt.yticks(fontsize=15)
        # plt.title("Two Targets Optimization")
        plt.savefig(f'../{name}_gaImages/generation_{i+1}.jpg',dpi=600)
        # plt.show()

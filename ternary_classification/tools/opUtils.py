...
# 矫正分类器模型重建
def modelling_revised_calibration(X_train, X_train_val, y_train, y_train_val, X_test, y_test, model, name=None):
    # 训练模型
    clf = model.fit(X_train, y_train)
    # 进行classifier的矫正
    ...
    # before calibration
    multiclass_confusion_matrix_plot(y_test=y_test, y_pred=y_pred, name= (name+'before'))
    # after calibration
    multiclass_confusion_matrix_plot(y_test=y_test, y_pred=y_cal_pred, name=(name+'calibration'))
    # 绘制模型多分类AUROC曲线
    n_classes = 3
    # (1)未矫正前
    ...
    # (2)矫正后
    # 绘制预测概率+矫正概率+矫正器网格
    # multi_class_calibration_grid(X_test, y_test, clf, cal_clf,name=name)
    # 评估模型
    print("=============================")
    selected_columns = X_train.columns.values
    print(selected_columns)
...
def multi_class_auroc_compare(n_classes, y_test, y_pred, y_cal_pred, name=None):
    ...
    plt.grid()
    plt.savefig("../images/multi_class_auroc_" + name + "_" + str(ts) + ".png", dpi=600)
...
def multi_class_calibration_grid(X_test, y_test, clf, cal_clf, name=None):
    ...
    t = round(time.time())
    plt.savefig("../images/multiclass_calibration_" + name + str(t) + ".jpg", dpi=600)
    ...
    plt.xlim(-0.05, 1.05)
    plt.ylim(-0.05, 1.05)
    plt.savefig("../images/Learned_sigmoid_calibration_map_" + name + str(t) + ".jpg", dpi=600)
